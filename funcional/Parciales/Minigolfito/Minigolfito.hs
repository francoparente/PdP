...
Para hacerlo más interesante, los padres de los niños hicieron una apuesta:
el padre del niño que no gane deberá cortar el césped del otro usando un vestido de su esposa.
...
cantidadSuperada :: [Obstaculo] -> Tiro -> Int
cantidadSuperada [] tiro = 0
cantidadSuperada (obstaculo:obstaculos) tiro
  | puedeSuperar obstaculo tiro = 1 + cantidadSuperada obstaculos (efectoAlSuperar obstaculo tiro)
  | otherwise = 0
...
paloMasUtil jugador obstaculos = maximoSegun (cantidadSuperada obstaculos.golpe jugador) palos
...
pierdenApuesta puntajes = (map (padre.jugadorTorneo) . filter (not.gano puntajes)) puntajes

gano :: [(Jugador, Puntos)] -> (Jugador, Puntos) -> Bool
gano puntajes puntajeJugador = (all ((< puntosGanados puntajeJugador).puntosGanados) . filter (/= puntajeJugador)) puntajes